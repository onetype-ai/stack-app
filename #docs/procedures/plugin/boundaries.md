...
and reach the plugin's services through it, so they run anywhere. `use()` is the
...
A method given someone else's `ctx` must not emit: an event carries the identity
of the context it went through, so the kernel refuses it. Emitting belongs to
the service, which holds its own.

It is a plugin's only importable file; one holding nothing but types and a hook
...
A listener that throws reaches neither the emitter nor the others.
...
settings panels are contributions, never imports. The opener defines the payload
schema; one failing it is rejected at startup.
...
one plugin uses stays inside it until a second needs it.
