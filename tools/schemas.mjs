#!/usr/bin/env node
//
// The surface this project's kit offers, written out as a flat list of
// signatures: what a plugin here may call.
//
// Generated from the kit's built .d.ts in node_modules rather than from its
// source, so what it shows is what this project actually receives. Run it
// after installing or relinking the kit.
//

import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { join } from "node:path";

const OUT = "schemas.md";

function entryPoints()
{
    const manifest = JSON.parse(readFileSync("package.json", "utf8"));
    const name = Object.keys(manifest.dependencies ?? {}).find((one) => one.endsWith("-kit"));

    if (name === undefined)
    {
        throw new Error("No dependency ends in -kit, so there is no surface to read.");
    }
    ...
    for (const [path, target] of Object.entries(kit.exports ?? {}))
    {
        const types = typeof target === "string" ? null : target.types ?? target.import?.types ?? null;
        const file = types === null ? null : join(root, types);

        if (file !== null && existsSync(file))
        {
            entries.push({
                name: path === "." ? kit.name : `${kit.name}/${path.replace("./", "")}`,
                file,
            });
        }
    }
    ...
    if (entries.length === 0)
    {
        throw new Error(`${name} published no types to read. Build it first.`);
    }
