...
// would read them.
//
//   new Packer({ at: "src/plugins", demo: ["documents"] }).run(argv)
//
...
export class Packer
{
    ...
    pack(asked)
    {
        ...
        const files = this.whole
            ? this.walk(this.folder).sort((first, second) => this.weigh(first) - this.weigh(second) || first.localeCompare(second))
            : names.flatMap((name) => this.walk(this.pathFor(name)).sort((first, second) =>
            {
        ...
        const held = new Set(files.map((one) => relative(this.root, one)));
        ...
        if (this.whole)
        {
            const inside = this.file.startsWith(`${this.folder}${sep}`);
        ...
        else
        {
            for (const name of names)
            {
                rmSync(this.pathFor(name), { recursive: true, force: true });
            }
        }
        ...
        console.log(`those ${String(held.size)} files are now gone from disk. \`unpack\` writes them back.`);
    }
